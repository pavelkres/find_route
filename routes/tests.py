...
class AllTestCase(TestCase):
    ...
    def test_model_train_duplicate(self):
        ...
        train = Train(name='t1', from_city=self.city_A, to_city=self.city_B, travel_time=10)
        with self.assertRaises(ValidationError):
            train.full_clean()

    def test_model_train_duplicate2(self):
        '''Тестирование возникновения ошибки при создании дубля поезда'''
    ...
    def test_message_error_from_cities(self):
        ...
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Маршрут через нужные города не найден', 1, 200)
